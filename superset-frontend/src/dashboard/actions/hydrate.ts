...
export const hydrateDashboard =
  ...
  (dispatch: AppDispatch, getState: GetState): AnyAction => {
    ...
    // Seed Zustand store with hydrated values. Captured into a named object so
    // the exact same seed can be cached and replayed by useDiscardChanges
    // (HYDRATE_DASHBOARD only resets the Redux reducers, not the Zustand store).
    const zustandStateSeed = {
      ...
      // Match upstream: bar starts closed, the useNativeFilters mount effect
      // opens it when the dashboard has filters.
    ...
    // Seed layout Zustand store. The seed is the initial state — there is
    // nothing to undo back to — so clear the zundo history afterwards,
    // otherwise the empty `{}` layout lingers as a bogus past state.
    useDashboardLayoutStore.getState().setLayout(dashboardLayout.present);
    useDashboardLayoutStore.temporal.getState().clear();

    // Seed the dashboard's own chart metadata. Consumers (Chart, FilterScope,
    // …) read it from here; the "add chart" panel uses useSlicesQuery instead.
    useDashboardSlicesStore
      .getState()
      .setSlices(slices as Record<number, Slice>);

    // Seed the filter Zustand stores. hydrateDataMask mirrors the dataMask
    // reducer's HYDRATE_DASHBOARD branch; hydrateNativeFilters mirrors the
    // nativeFilters reducer's.
    useDataMaskStore
      .getState()
    ...
    const hydrateData = {
      ...
      nativeFilters,
      dashboardLayout,
      // Not consumed by any reducer — carried so useDiscardChanges can replay
      // the Zustand state seed (HYDRATE_DASHBOARD doesn't touch Zustand).
      zustandStateSeed,
    };

    // Seed the dashboardInfo Zustand store. Mirrors the dashboardInfo reducer's
    // HYDRATE_DASHBOARD branch — readers consume it from here.
    useDashboardInfoStore
      .getState()
