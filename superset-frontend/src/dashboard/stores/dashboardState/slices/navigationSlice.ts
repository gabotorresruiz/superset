...
export interface NavigationSlice {
  ...
  /**
   * Merges a tab switch into the active/inactive sets — `activeTabs` and
   * `inactiveTabs` are deltas (from findTabsToRestore), so sibling tab
   * groups must be preserved rather than replaced.
   */
  applyActiveTab: (
    activeTabs: string[],
    inactiveTabs: string[],
...
export const navigationInitialState = {
  ...
  // Upstream had no initial value (undefined → `?? false`); the mount effect in
  // useNativeFilters opens the bar when the dashboard has filters.
