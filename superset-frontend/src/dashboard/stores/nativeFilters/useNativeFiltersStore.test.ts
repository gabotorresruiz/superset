...
test('setFiltersConfigComplete replaces filters from the backend response', () => {
  store.setState({ filters: { stale: filter('stale') } });
  store
    .getState()
    .setFiltersConfigComplete([
      filter('A', { chartsInScope: [1], tabsInScope: ['T1'] }),
    ]);
  // backend response is authoritative — the stale filter is dropped
