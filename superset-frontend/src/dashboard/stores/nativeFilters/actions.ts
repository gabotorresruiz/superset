...
/**
 * Imperative domain API for the native filters client state — for use outside
 * render (event handlers, effects, non-React modules). Components import these
 * from `src/dashboard/stores` instead of calling `useNativeFiltersStore`
...
export const setFocusedFilter = (id: string): void =>
  useNativeFiltersStore.getState().setFocusedFilter(id);

export const unsetFocusedFilter = (): void =>
  useNativeFiltersStore.getState().unsetFocusedFilter();

export const setHoveredFilter = (id: string): void =>
  useNativeFiltersStore.getState().setHoveredFilter(id);

export const unsetHoveredFilter = (): void =>
  useNativeFiltersStore.getState().unsetHoveredFilter();

export const setHoveredChartCustomization = (id: string): void =>
  useNativeFiltersStore.getState().setHoveredChartCustomization(id);

export const unsetHoveredChartCustomization = (): void =>
