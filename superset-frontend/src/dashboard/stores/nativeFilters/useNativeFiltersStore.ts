...
/**
 * Builds the native-filters state from a filter config, preserving any
 * existing filters. Used by hydration and the in-scope-status update.
 */
...
function mergeFilterChanges(
  ...
  // Backend response is authoritative — deleted filters are simply absent.
  const newFilters: Record<string, FilterEntry> = {};
...
export interface NativeFiltersStore extends ExtendedNativeFiltersState {
  ...
  setFiltersConfigComplete: (filterChanges: FilterEntry[]) => void;
  ...
  /** Replaces the filters from the dashboard hydration payload. */
