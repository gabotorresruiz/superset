...
/**
 * Pure construction of a dashboard's layout tree and chart data structures
 * from a persisted `position_data` map plus the dashboard's chart payloads.
 * Slices arriving from Explore that have no layout node yet are appended into
 * fresh rows. Extracted from the `hydrateDashboard` thunk so it can be unit
 * tested in isolation.
 */
