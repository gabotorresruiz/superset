...
/**
 * Enable Save as a title is typed, and disable it again if the title is reverted
 * to `savedTitle` - but only clear when this edit was the sole change, so other
 * pending edits keep the dashboard dirty.
 */
...
/**
 * Reset the marker when a title edit ends (blur), so the next edit re-reads the
 * dashboard's clean state instead of clearing on a stale "was clean" value.
 */
