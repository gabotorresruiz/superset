...
test('preserves session-only fields across discard', () => {
  ...
  // Live store reflects user actions taken outside edit mode: navigated to
  // a tab (which marks its sibling inactive), set a session-only refresh
  // interval, opened the filter bar.
  useDashboardStateStore.setState({
    hasUnsavedChanges: true,
    editMode: true,
    refreshFrequency: 10,
