...
export function useDiscardChanges(dashboardId: number | string) {
  ...
  return useCallback((): boolean => {
    ...
    // The cached payload carries pristine chart entries; replaying them as-is
    // would drop every chart's query result and force a full re-fetch. Reuse
    // the live entries (with responses) for the original chart set so discard
    // stays in-place. (Charts intentionally remain on Redux, shared with Explore.)
    ...
    if (cached.zustandStateSeed) {
      // Session-only fields are session-scoped, not dashboard-scoped: preserve
      // the live values so discard doesn't undo user navigation or a
      // session-only auto-refresh setting.
      const live = useDashboardStateStore.getState();
      useDashboardStateStore.setState({
        ...cached.zustandStateSeed,
        // A session-only refresh interval (viewer-set) survives discard; a
        // persistent one is a dashboard edit and reverts to the seed.
        ...
        // activeTabs/inactiveTabs are a paired set; preserve both so discard
        // keeps live tab navigation instead of resetting inactiveTabs.
    ...
    // (chartsInScope) the snapshot predates; config changes reload, not discard.
