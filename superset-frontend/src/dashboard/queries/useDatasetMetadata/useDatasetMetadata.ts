...
/**
 * Searches datasets (paginated) through the shared React Query cache. Imperative
 * (used by the dataset AsyncSelect's loader), so it goes through fetchQuery
 * rather than a hook, keeping all data access in the queries/ layer. Generic over
 * the row shape so callers get typed results without an external cast.
 */
export function fetchDatasetList<T = JsonObject>(
  query: string,
): Promise<DatasetListResult<T>> {
  return queryClient.fetchQuery({
    queryKey: datasetKeys.list(query),
    queryFn: async () => {
...
/**
 * Reads dataset metadata (columns / metrics) through TanStack Query so repeated
 * reads of the same dataset + projection dedupe and share one cache. Replaces
 * the legacy cachedSupersetGet calls scattered across the filter config UI.
 */
