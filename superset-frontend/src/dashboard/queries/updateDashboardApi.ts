...
/**
 * Applies the result of a successful dashboard-metadata save: pushes the fresh
 * metadata into the store (the single source of truth), optionally syncs the
 * overwrite-protection timestamp, rebaselines the discard snapshot, and
 * invalidates the detail query. Shared by every metadata-save mutation so the
