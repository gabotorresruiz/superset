...
/**
 * Zustand auto-mock for Jest.
 *
 * Automatically resets all Zustand stores to their initial state between tests.
 * This prevents state leakage across tests without requiring manual cleanup.
 * Handles stores with middleware (devtools, subscribeWithSelector, temporal/zundo).
 *
...
// Re-export createStore from zustand/vanilla so middleware like zundo's
// temporal can resolve it when importing from the mocked 'zustand' module.
export { createStore } from 'zustand/vanilla';

// Re-export the real useStore hook (used to subscribe to vanilla stores such
// as zundo's temporal store).
...
const createUncurried = <T>(stateCreator: StateCreator<T>) => {
  ...
  storeResetFns.add(() => {
    ...
    // Clear temporal (zundo) history if present
